<template>
	<view class="uni-table-scroll" :class="{ 'table--border': border, 'border-none': !noData }">
		...
		<!-- #endif -->
		<!-- #ifndef H5 -->
		<scroll-view scroll-x="true" scroll-y="true" style="width: 100%;overflow: auto;" v-if="landscape">
		...
		<view class="uni-table" :style="{ 'min-width': minWidth + 'px' }" :class="{ 'table--stripe': stripe }" v-else>
			<slot></slot>
			<view v-if="noData" class="uni-table-loading">
				<view class="uni-table-text" :class="{ 'empty-border': border }">{{ emptyText }}</view>
			</view>
