				...
				<view class="input-value" :class="{'input-value-border': border}">
					...
					<scroll-view v-else-if="inputSelected.length" class="selected-area" scroll-x="true">
						<view class="selected-list">
							<view class="selected-item" v-for="(item,index) in inputSelected" :key="index" v-if="!onlyShowLast">
								<text>{{item.text}}</text><text v-if="index<inputSelected.length-1"
									class="input-split-line">{{split}}</text>
							</view>
							<view class="selected-item" v-else>
								<text>{{ preText + inputSelected[inputSelected.length-1].text}}</text>
							</view>							
						</view>
					</scroll-view>
