										...
										<view
											...
											<template v-if="item.type === 'selection'">
												<view class="checkbox-item">
													<tableCheckbox :indeterminate="indeterminate" :checked="checkedAll" @checkboxSelected="checkboxSelectedAll"></tableCheckbox>
												</view>
											</template>
											<template v-else>
												{{ item.label }}
					...
					<view class="zb-table-fixed">
						<view class="zb-table-tbody">
							<view
								...
								@longpress="(e) => trLongPress(e, item)"
								@touchstart="trTouchStart"
								hover-class="hover-class"
								v-for="(item, index) in transData"
								:key="item.key">
								<table-hover-button
									:isShowHoverBtn="item.isShowHoverBtn"
									:offsetLeft="offsetLeft"
									:triggerUpdate="triggerUpdate"
									:trInfo="item"
									@delete="deleteTr"></table-hover-button>
								<view
									:style="[
										{
									...
									<template v-if="ite.type === 'operation'">
										<view style="display: flex; align-items: center; height: 100%">
											<view
												...
												<template v-else>
													<button :class="ren.class || ''" :type="ren.type || 'primary'" :size="ren.size || 'mini'">{{ ren.name }}</button>
												</template>
											</view>
										</view>
									</template>
									...
									<template v-else>
										...
										<!-- <view class="fst-td-content">
											<text class="fst-td-content-text">{{ ite.filters ? itemFilter(item, ite) : formatterAction(item, ite, index, i) }}</text>
											<template v-if="calcIsShowBottomText(i)">
												<view class="bottom-text">{{ item.code }}</view>
											</template>
										</view> -->
									</template>
								</view>
						...
						<view class="load-more" v-if="loadMore">
							<u-loadmore :status="status" :load-text="loadText" @loadmore="$emit('loadmore')" />
						</view>
					</view>
				</scroll-view>
				<table-h5-summary
			...
			<view class="zb-table-fixed-left" v-if="isFixedLeft" :style="{ height: `calc(100% - ${scrollbarSize}px)` }">
				<template v-if="showHeader">
					<view class="zb-table-header" style="display: flex">
						<view class="item-tr" style="" hover-class="hover-class" @click.stop="rowClick(item, index)" v-for="(item, index) in fixedLeftColumns" :key="index">
							<view
								:style="{
									width: `${item.width ? item.width : '100'}px`,
									borderRight: `${border ? '1px solid #e8e8e8' : ''}`,
						...
						<view class="zb-table-tbody">
							<view
								:class="[
									...
									item.isCurCompare ? 'current-compare-row' : '',
									!columns[0].type ? 'fst-col-no-val' : 'fst-col-exist-val',
								]"
...
<script>
	...
	import TableHoverButton from './components/table-hover-button.vue'
	import { cmpIndustryNum } from '../../../../subPackages/stock/compare-industry/compare-industry'
	// #ifdef H5
	...
	export default {
		...
		computed: {
			calcIsShowBottomText() {
				return (i) => {
					if (!this.columns[0].type) {
						return this.isShowBottomText && i === 0 ? true : false
					} else {
						return this.isShowBottomText && i === 1 ? true : false
					}
				}
			},
			loadMoreHeight() {
			...
			transColumns() {
				...
				this.columns.forEach((item, index) => {
					...
					if (item.defaultSort && !this.isSorted) {
			...
			transData() {
				...
				this.tableData = this.$util.deepCopy(this.data)
				this.data.forEach((item, index) => {
					this.$set(item, 'isShowHoverBtn', false)
					if (flag) {
						if (item.checked) {
		...
		methods: {
			...
			checkboxSelected(e, item, isActiveTrigger = true) {
				// #ifdef H5 || APP-PLUS
				...
				// #endif
				if (isActiveTrigger) {
					this.$emit('toggleRowSelection', e.checked, item)
				}
			},
			itemFilter(item, ite) {
			...
			sortAction(item, index) {
				...
				this.columns.forEach((columnsItem, index, arr) => {
					if (columnsItem.sorterMode && item.name != columnsItem.name) {
						this.$set(arr[index], 'sorterMode', '')
					}
				})
			...
			sortData(item) {
				...
				if (item.sorterMode === '_asc') {
					this.data.sort((a, b) => {
						...
						if (typeof a[key] != 'string') a[key] = String(a[key])
						if (a[key].indexOf(',') != -1) {
							if (typeof b[key] != 'string') b[key] = String(b[key])
							if (b[key] == '--') return 1
							let a1 = Number(a[key].replace(/,/g, ''))
							let b1 = Number(b[key].replace(/,/g, ''))
							return a1 - b1
						}
						this.isSorted = true
					})
			...
			trLongPress(e, item) {
				if (item.isCurCompare) return
				this.data.forEach((dataItem, index, arr) => {
					// this.$set(arr[index],'isShowHoverBtn', item.name === dataItem.name? true: false)
					arr[index].isShowHoverBtn = item.name === dataItem.name ? true : false
				})
				if (e.target.x > (this.trWidth * 3) / 4) {
					this.offsetLeft = (this.trWidth * 3) / 4
				} else {
					this.offsetLeft = e.target.x + 10
				}
				this.triggerUpdate = !this.triggerUpdate
			},
			trTouchStart(e) {
				this.data.forEach((dataItem, index, arr) => {
					arr[index].isShowHoverBtn = false
				})
				this.triggerUpdate = !this.triggerUpdate
			},
			deleteTr(name) {
				this.$emit('deleteTr', name)
			},
...
<style lang="scss" scoped>
	...
	.fst-col-exist-val {
		.item-td:nth-of-type(3) {
			line-height: 2.2em;
		}
	}
	.fst-col-no-val {
		.item-td:not(:first-of-type) {
			line-height: 2.4em;
		}
	}

	.item-td:nth-of-type(n + 3) {
