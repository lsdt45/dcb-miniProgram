...
// import H5_HQChart from '@/pages/stock-trend/static/umychart.uniapp.h5/umychart.uniapp.h5.js'
...
import {
	JSCommon
} from "@/pages/stock-trend/static/umychart.uniapp/umychart.wechat.3.0.js";
import {
	MARKET_SUFFIX_NAME
} from '@/pages/stock-trend/static/umychart.uniapp/umychart.coordinatedata.wechat.js'
import {
	JSCommonHQStyle
...
HQData.NetworkFilter = function(data, callback, then) {
	switch (data.Name) {
		case 'MinuteChartContainer::RequestMinuteData': //分时图数据对接
			HQData.RequestMinuteData(data, callback, then);
			break;
...
HQData.RequestMinuteData = function(data, callback, then) {
	data.PreventDefault = true;
	var dayCount = data.Request.Data.daycount;

	then.$api.post('/Report/GetDataThirdParty', {
		Code: then.EmSymbol,
		DataType: "分时",
	}, {}, false).then(recvData => {
		HQData.RecvMinuteData(recvData, callback, then);
	})
}

HQData.RecvMinuteData = function(recvData, callback, then) {
	...
	var symbolUpper = then.Symbol.toUpperCase();
...
HQData.RequestMinuteDaysData = function(data, callback, then) {
	data.PreventDefault = true;
	var dayCount = data.Request.Data.daycount;
	var symbolUpper = then.Symbol.toUpperCase();
	...
	then.$api.post('/Report/GetDataThirdParty', {
		...
		if (MARKET_SUFFIX_NAME.IsSHFE(symbolUpper) || MARKET_SUFFIX_NAME.IsDCE(symbolUpper) ||
			...
			HQData.RecvMinuteDaysData(recvData, callback, then);
		}
	})
...
HQData.RecvMinuteDaysData = function(recvData, callback, then) {
	var data = recvData.data;
	var aryDayData = [];
	var itemDay = null;
	...
	var xDatetime = JSChart.GetMinuteTimeStringData().GetTimeData(then.Symbol);
	...
	for (var j = 0; j < data.trends.length; ++j) {
		...
		itemDay.minute.push([stockItem.time, stockItem.open, stockItem.high, stockItem.low, stockItem.price,
			stockItem.vol, stockItem.amount, stockItem.avprice, stockItem.date
		]);
	}
...
HQData.GetMinuteApiUrl = function(symbol, dayCount) {
	...
	var url =
		`https://push2his.eastmoney.com/api/qt/stock/trends2/get?fields1=f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f11,f12,f13&fields2=f51,f52,f53,f54,f55,f56,f57,f58&secid=${internalSymbol.Market}.${internalSymbol.Symbol}&ndays=${dayCount}&iscr=0&iscca=0`
...
HQData.RequestHistoryData = function(data, callback,then) {
	data.PreventDefault = true;
	...
	then.$api.post('/Report/GetDataThirdParty', {
		...
		HQData.RecvHistoryData(recvData, callback, then);
	})
...
HQData.RecvHistoryData = function(recvData, callback, then) {
	var data = recvData.data;
	...
	hqChartData.symbol = then.Symbol;
