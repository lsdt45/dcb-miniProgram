	...
	<scroll-view class="report-information-wrapper" scroll-y="true" show-scrollbar="true" @scrolltolower="reachBottom">
		<view class="report-list">
			<uni-list class="report">
				<uni-list-item class="list-item" v-for="(item, index) in reportInfoData" :key="index" :clickable="true"
					@click="toReport(item.url)">
...
<script>
	export default {
		...
		methods: {
			//获取第一页资讯
			getNewsPage() {
				this.$api.post("/Report/GetInformation", {
					...
					if (!resp.data || resp.data.length == 0) {
						...
						this.reportInfoData.forEach(item => {
							item.createTime = this.$util.FormatDate(new Date(item.createTime),
								'yyyy-MM-dd')
						})
					}
				}).catch(err => {
