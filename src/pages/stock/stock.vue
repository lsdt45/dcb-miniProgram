<template>
	<view class="stock-wrapper">
		<dcb-nav-bar v-if="isShowTopNavTab" :title="stockFullName" :leftIconType="back" :stock-home="true">
		</dcb-nav-bar>
		...
		<view class="stock-content">
			...
			<view class="stock-content-financial-analysis">
				...
				<scroll-view class="table-charts single-table" v-if="curAnalysisNode.type == 'single'" scroll-x="true"
					scroll-y="true" :style="tableChartViewHeight">
					<table-charts :chartsInfoProp="chartsInfo" :chartsId="chartsId" :rptType="rptTypeProp"
						ref="singleTable">
					</table-charts>
				</scroll-view>
				<!-- 多图表显示 -->
				<scroll-view class="table-charts multi-table" v-else scroll-x="true" scroll-y="true"
					:style="tableChartViewHeight" @scroll="scroll" :scroll-top="scrollTop.new">
		...
		<view class="tabel-charts-rotate">
			<u-mask :show="isShowRotate">
				<view :class="maskClass" :style="rotateCanvasHeight" v-if="isShowRotate">
					<qiun-data-charts :type="chartsShowType" :chartData="chartsData_rotate" :opts="opts_rotate"
						:inScrollView="true" :canvasId="canvasId" :ontouch="true" :onzoom="true" :canvas2d="true"
						tooltipFormat="formatTooltipData" :reshow="isShowRotate" />
				</view>
...
<!-- 		<view class="tabel-charts-rotate">
			<u-mask :show="isShowRotate_table">
					<view class="table-charts table" :class="maskClass_table" v-if="isShowRotate_table">
						<dcb-table :chartsData="chartsData_rotate" :fstTableTh="fstTableTh"></dcb-table>
					</view>
				<view class="iconfont icon-close" @click="closeRotateMode"></view>
			</u-mask>
		</view> -->
...
<script lang="ts">
	...
	import TableCharts from '@/pages/stock/table-charts/table-charts.vue'
	import PdfReport from '@/pages/stock/pdf-report.vue'
	import DcbTable from '@/pages/stock/table-charts/dcb-table.vue'
	import {
		mapState
	} from 'vuex'
	export default {
			...
			async selectTemplate(item) {
				...
				setTimeout(() => {
					...
					if (this.curAnalysisNode.type == 'single') {
						...
						chart.opts.legend.isShowAllLegend = false
						chart.opts.legend.isReload = true
		...
		mounted() {
			this.curAnalysisNode.id = this.analyseNodeProp.id
			this.curAnalysisNode.topic = this.analyseNodeProp.topic
			this.initData()
		},
	}
</script>
