<template>
	<view class="stock-search-wrapper">
		<uni-nav-bar title="股票搜索" left-icon="left" :height="navBarHeight" backgroundColor="#2C72EC" color="#FFF"
			:fixed="true" @clickLeft="back">
		</uni-nav-bar>
		<view class="stock-search__body">
			<view class="stock-search-header">
				<u-search placeholder="股票" v-model="keyWord" shape="square" action-text="清空" @change="startSearch"
					:clearabled="false" :action-style="{color: '#2c72ec'}" @custom="handleResultShow">
				</u-search>
			</view>
			<view class="search-result" v-if="isShowResult">
				<uni-transition :mode-class="['fade', 'zoom-in']" :show="isResultExit">
					<uni-list :border="false">
						<uni-list-item v-for="item in searchResultData" :key="item.seccode" :border="false" clickable
							@click="toSpecifyStock(item, 'search')">
							<template v-slot:header class="search-result__item-name">
								{{ item.secname + "(" + item.seccode + ")" }}
							</template>
							<template v-slot:footer>
								<button class="add-stock-btn" type="default" size="mini"
									@click.stop="addOptionalStock(item)">
									{{ addBtnText(item) }}
								</button>
							</template>
						</uni-list-item>
			...
			<view class="search-history" v-else>
				...
				<view class="search-history-label">
					<u-tag class="tag" v-for="(item, index) in historyResult" :key="index" :text="item.secname"
						@click="toSpecifyStock(item, 'tag')" bg-color="#f2f2f2" border-color="#f2f2f2"
						color="#797979" />
				</view>
			</view>
		</view>
