...
<template>
	<view class="stock-search-wrapper">
		<!-- #ifdef MP -->
		<uni-nav-bar title="股票搜索" left-icon="left" :height="navBarHeight" backgroundColor="#2C72EC" color="#FFF" :fixed="true" @clickLeft="back"></uni-nav-bar>
		<!-- #endif -->
		<!-- #ifdef H5 -->
		<uni-nav-bar title="股票搜索" left-icon="left" height="50px" backgroundColor="#2C72EC" color="#FFF" :fixed="true" @clickLeft="back"></uni-nav-bar>
		<!-- #endif -->
		<view class="stock-search__body">
			<view class="stock-search-header">
				<u-search
					...
					@custom="handleResultShow"></u-search>
			</view>
			<view class="search-result" v-if="isShowResult">
				<uni-transition :mode-class="['fade', 'zoom-in']" :show="isResultExit">
					<uni-list :border="false">
						<uni-list-item v-for="item in searchResultData" :key="item.seccode" :border="false" clickable @click="toSpecifyStock(item, 'search')">
