...
/***/ (function (module, exports, __webpack_require__) {
    ...
    var PDFViewerApplication = {
      ...
      setInitialView: function setInitialView(storedHash) {
        ...
        //解析url参数
        var c_url = decodeURIComponent(window.location.href);
        if (c_url.indexOf("&") && c_url.indexOf("=")) {
          ...
          //是否为校正模式
          if (c_urlArray['isCheckModel']
            && c_urlArray['isCheckModel'].toLowerCase() == 'true') {
            document.getElementById('viewer').style.cssText = "background-color:#ccc;overflow: scroll;";
          }
          //页面位置定位
          if (c_urlArray['top']) {
            var pdfScaleValue = parseFloat(document.getElementById("scaleValue").innerHTML);
            document.getElementById('viewerContainer').scrollTop = document.getElementById('viewerContainer').scrollTop + (parseFloat(c_urlArray['top']) * pdfScaleValue);
          }
        }
        var self = this
        var pdfIndex = c_urlArray['index']
        //页码改变click
        document.getElementById("changePage").onclick= function(){
          // var initPage = window.parent.document.getElementById("initPage").innerHTML
          var initPage = document.getElementById("initPage").innerHTML
            if(initPage && parseFloat(initPage)>0){
              self.pdfViewer.currentPageNumber =  initPage* 1;
            }
            document.getElementById("changeSite").click();
        }
        //复制文本
        ...
        value: function _enter() {
          ...
          // this.container.classList.add(ACTIVE_SELECTOR);
           setTimeout(function () {
             ...
             _this3.pdfViewer.currentScaleValue = 'page-actual';
           }, 0);
    ...
    var BaseViewer = function () {
      ...
      _createClass(BaseViewer, [{
        ...
        value: function _setScaleUpdatePages(newScale, newValue) {
          ...
          document.getElementById("scaleValue").innerHTML = newScale
    ...
    var TextLayerBuilder = function () {
      function TextLayerBuilder(_ref) {
        ...
        if (c_url.indexOf("&") && c_url.indexOf("=")) {
         ...
         // this.keyText = c_urlArray['keyText']
          //this.isCheckModel = 
          
          //= c_urlArray['isCheckModel']
          //  && c_urlArray['isCheckModel'].toLowerCase() == 'true'
          this.pdfIndex = c_urlArray['index']
          //this.offSetTop = c_urlArray['top']
          //this.keyPage = parseInt(c_urlArray['page'])
        }
        ...
        this.pageHeight = []
        var dom = document.getElementById("viewer").getElementsByClassName("page");
        for(var i = 0 ;i < dom.length;i++)
        {
          this.pageHeight.push( +dom[i].style.height.replace('px',''));
        }
        this.textDivs = [];
        this.findController = findController;
      ...
      _createClass(TextLayerBuilder, [{
        ...
        value: function _finishRendering() {
          ...
          if(parentDom.getElementById("pdfLoadFinish"))
          {
            //加载完之后，通知外层
            parentDom.getElementById("pdfLoadFinish").click();
          }
          var isCheckModel = parentDom.getElementById("checkModel"+this.pdfIndex).innerHTML == "true"
          textLayerDivList.push({page: this.pageNumber,textDivs: this.textDivs});
          this.keyText =  parentDom.getElementById("initKey"+this.pdfIndex).innerHTML
          this.offSetTop =  parentDom.getElementById("inittop"+this.pdfIndex).innerHTML == "-1"?null:parentDom.getElementById("inittop"+this.pdfIndex).innerHTML
          this.keyPage =  parentDom.getElementById("initPage"+this.pdfIndex).innerHTML
          var notfind = true
          //所有行信息
          var textDivs = this.textDivs
          //循环所有行
          for (var i = 0; i < this.textDivs.length; i++) {
            var element = this.textDivs[i]
            element.pageNumber = this.pageNumber
            if (element.innerHTML.trim().length == 0) {
              continue;
            }
            //用关键字匹配位置
            if (this.keyText && this.keyText.length > 0 && notfind && !this.offSetTop && this.pageNumber == this.keyPage) {
              //找到同一行数组
              var keyOnOneLine = []
              this.textDivs.forEach(function(t){
                if(t.offsetTop >= element.offsetTop - 5 && t.offsetTop <= element.offsetTop + 5)
                {
                  keyOnOneLine.push(t);
                }
              })
              var textContent = "";
              if (keyOnOneLine && keyOnOneLine.length > 0) {
                keyOnOneLine.forEach(function(oneLineItem){
                  textContent += oneLineItem.textContent.trim()
                })
              }
              //对比文本，若文本相等
              if (textContent == this.keyText || element.textContent == this.keyText) {

                for (var j = 0; j < keyOnOneLine.length; j++) {
                  //校正模式
                  if (isCheckModel) {
                    keyOnOneLine[j].className = 'checkModel checkActive'
                    if (j == keyOnOneLine.length - 1 && isCheckModel) {
                      window.parent.document.getElementById('keyText'+self.pdfIndex).innerHTML = this.keyText
                    }
                  }
                }
                var heightTop = 0
                for(var page = 0 ; page < this.pageIdx;page++)
                {
                  heightTop += (this.pageHeight[page] +11);
                }
                //跳到对应位置
                document.getElementById('viewerContainer').scrollTop = heightTop + element.offsetTop;
                notfind = false;
              }
            }

            //设置双击事件，用于校正时设置或取消起始点
            element.ondblclick = function (event) {
              //判断是否为校正模式
              if(parentDom.getElementById("checkModel"+self.pdfIndex).innerHTML == "true"){
                var el = event.target;
                if (el.className.indexOf('highlight') != -1) {
                  el = el.parentNode;
                }
                var oneLinetextContent = ""
                var elementOnOneLine = []
                //找到同行文本，作为起始点keytext
                textDivs.forEach(function(t){
                  if(t.offsetTop >= el.offsetTop - 5 && t.offsetTop <= el.offsetTop + 5)
                  {
                    elementOnOneLine.push(t);
                  }
                })

                if (elementOnOneLine && elementOnOneLine.length > 0) {
                  elementOnOneLine.forEach(function(oneLineItem){
                    oneLinetextContent += oneLineItem.textContent.trim()
                  })
                  var pdfScaleValue = parseFloat(document.getElementById("scaleValue").innerHTML);
                  parentDom.getElementById("siteInfo"+self.pdfIndex).innerHTML = (elementOnOneLine[0].offsetTop / pdfScaleValue)
                  parentDom.getElementById("keyText"+self.pdfIndex).innerHTML = oneLinetextContent
                  document.getElementById('pageNumberValue').value = elementOnOneLine[0].pageNumber
                  parentDom.getElementById("setAsStart"+self.pdfIndex).click()
                }
              }
            }
          }
          //校正模式中高亮起始行
          if (this.offSetTop && notfind && isCheckModel && this.pageNumber == this.keyPage) {
            var pdfScaleValue = parseFloat(document.getElementById("scaleValue").innerHTML);
            this.offSetTop = parseFloat(this.offSetTop) * pdfScaleValue

            var topOnOneLine = []
            //上下5个像素认为是同一行
            this.textDivs.forEach(function(t){
              if(t.offsetTop >= parseInt(this.offSetTop) - 5 && t.offsetTop <= parseInt(this.offSetTop) + 5)
              {
                topOnOneLine.push(t);
              }
            });
            if (topOnOneLine && topOnOneLine.length > 0) {
              for (var j = 0; j < topOnOneLine.length; j++) {
                topOnOneLine[j].className = ' checkActive'
                if (j == topOnOneLine.length - 1 && isCheckModel) {
                  window.parent.document.getElementById('keyText'+self.pdfIndex).innerHTML = this.keyText
                  // elementOnOneLine[j].innerHTML+="<strong style=\"font-size:14px\">已设置为起始点</strong>";
                }
              }
              notfind = false;
            }
          ...
          document.getElementById("changeSite").onclick= function(){
            ...
            var pdfScaleValue = parseFloat(document.getElementById("scaleValue").innerHTML);
            //top位置
            var inittop = window.parent.document.getElementById("inittop"+ self.pdfIndex).innerHTML
            //如果有top定位,直接跳到对应位置
            if(inittop && parseFloat(inittop) >0){
               document.getElementById('viewerContainer').scrollTop = document.getElementById('viewerContainer').scrollTop + (parseFloat(inittop) * pdfScaleValue);
            }
            //页码和KeyText
            var initPage = window.parent.document.getElementById("initPage"+self.pdfIndex).innerHTML
            var initKey = window.parent.document.getElementById("initKey"+self.pdfIndex).innerHTML
            var initNotfind  = true //标识还未找到匹配行
            //有匹配页码
            if(initPage && parseFloat(initPage)>0){
              document.getElementById('pageNumber').value =  initPage * 1;
              //top跳转,直接返回
              if((inittop && parseFloat(inittop) >0) || (initKey.length == 0))
              {
                return;
              }
              //keytext匹配
              for(var j = 0; j < textLayerDivList.length; j++)
              {
                var nowTextDivs =  textLayerDivList[j]
                
                if(nowTextDivs.page == initPage * 1 && initNotfind)
                {
                    
                  for (var i = 0; i < nowTextDivs.textDivs.length; i++) {
                    var element = nowTextDivs.textDivs[i]
                    if (element.innerHTML.trim().length == 0) {
                      continue;
                    }
                    //用关键字匹配位置
                    if (initNotfind) {
                      var keyOnOneLine = []
                      nowTextDivs.textDivs.forEach(function(t){
                        if(t.offsetTop >= element.offsetTop - 5 && t.offsetTop <= element.offsetTop + 5)
                        {
                          keyOnOneLine.push(t);
                        }
                      ...
                      if (textContent == initKey || element.textContent == initKey) {
                        document.getElementById('viewerContainer').scrollTop = document.getElementById('viewerContainer').scrollTop + element.offsetTop;
                        initNotfind = false;
                      }
                    }
                  }
