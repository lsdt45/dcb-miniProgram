	...
	<view class="home-wrapper">
		...
		<view class="home-main">
			<view class="home-main__header">
				<u-search :placeholder="placeholderText" shape="square" action-text="自选股"
					:action-style="{ color: '#2C72EC' }" :disabled="true" input-align="center" @click="toSearch"
					@custom="toStockMarket">
				</u-search>
			...
			<view class="home-main__filter">
				...
				<view class="filter-body">
					<uni-tag class="filter-body__select" v-for="(item, index) in filterList" :key="index"
						:inverted="!item.select" type="primary"
			...
			<scroll-view class="home-main__report-scroll-wrapper" scroll-y="true" :style="stockCardAreaHeight">
				<view class="home-main__report">
					<view class="report-card" v-for="(item, index) in reportInfo" :key="index"
						@click="toAnalsisPage(item)">
						<uni-card :is-shadow="false">
							<template class="report-card__header" v-slot:title>
								<view class="title-text">
									<text class="stock-name"> {{ item.stockName }} </text>
									<text class="stock-code"> {{ item.code }} </text>
								</view>
								<image class="title-img" referrerPolicy="no-referrer" :src="setImgSrc(index)" mode="">
								</image>
							</template>
	...
	export default {
		...
		methods: {
			...
			getRecentStockInfo(
				...
				this.$api.post("/Report/GetRecentReports", param).then((resp) => {
					if (resp.status) {
						let data = resp.data;
						data.forEach((item, index) => {
							this.reportInfo.push({
								...
								announcementTime: this.$util.FormatDate(new Date(item
									.announcementTime), "yyyy-MM-dd"),
							});
						});
						this.$nextTick(() => {});
					}
