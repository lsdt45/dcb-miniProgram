	...
	<view class="pdf-report">
		...
		<view class="report-wrapper">
			<view class="report-index-main">
				<image class="report-wrapper__bg-img" referrerPolicy="no-referrer"
					src="https://infostar-2020-1301764129.file.myqcloud.com/miniProgram/static/iamge/ReportGuide111.png">
				</image>
				<view class="report-header">
					<view class="report-header__report-change">
						...
						<view :class="panelClass">
							<scroll-view class="report-header__scroll-view" :style="scrollStyle" scroll-y="true"
								@transitionend="hanldeTransition">
								<uni-list>
									<uni-list-item class="list-item" :class="{ selected: curSelectIndex == index }"
										v-for="(item, index) in rptListData" :key="item.id" clickable :border="false"
										@click="changeReport(item)">
										<template v-slot:header>
											<view>
												<text
													class="text">{{ item.year + "年" + $util.formatRptType(item.name) }}</text>
											</view>
										</template>
									</uni-list-item>
								</uni-list>
...
<script lang="ts">
	...
	import type { indexDataType } from "./pdf-report";
	export default {
		data() {
			return {
				rectData: "",
