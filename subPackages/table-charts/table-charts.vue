		...
		<view class="charts-header">
			...
			<view class="global-table-toolbar">
				...
				<view class="data-process-select" v-if="tagAnnualized.isExsit">
					<uni-data-select placeholder="数据处理" :clear="false" :localdata="dataProcessList" :fixedText="true"
						@change="dataProcess">
					</uni-data-select>
				</view>
			</view>
	...
	export default {
		...
		watch: {
			...
			curCmpList(newVal) {
				newVal.forEach((item) => {
					...
					this.$store.state.curCmpList.forEach((item) => {
		...
		methods: {
			...
			rotateMode(): void {
				...
				// 设置1s延时，避免渲染出错
				setTimeout(() => {
					this.$parent.$refs.chart.init(echarts, chart => {
						chart.setOption(tempOption)
					})
				}, 1000)
				setTimeout(() => {
					this.$parent.$refs.chart.resize()
				}, 1100)
			},
			/**
			 * description: 横屏模式-表格
			...
			rotateMode_table(): void {
				...
				this.$parent.chartsData_lanscape = this.$util.deepCopy(this.chartsData)
				this.$parent.chartsInfo_lanscape = this.$util.deepCopy(this.chartsInfo)
