<template>
	<view class="compare-industry__wrapper">
		<dcb-nav-bar :title="stockFullName" leftIconType="back" search>
		</dcb-nav-bar>
		...
		<view class="compare-industry__body">
			<zb-table :data="curCmpList" :columns="columns" :update="update" :fit="true" :isShowBottomText="true"
				@deleteTr="deleteTr">
			</zb-table>
		</view>
		<view class="btn-add" @click="openAddPage">
			<view class="btn-add-content">
				<text class="iconfont icon-newzoomin" ></text> 添加对比
			</view>
		</view>
	</view>
	<u-toast ref="toast" />
...
<style lang="scss">
	.compare-industry__wrapper {
		...
		.compare-industry__body {
			background-color: #fff;
			.item-th:nth-of-type(1), .item-td:nth-of-type(1) {
