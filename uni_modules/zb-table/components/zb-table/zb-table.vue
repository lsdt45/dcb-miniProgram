...
<!--                          <tableCheckbox
          ...
          <view class="zb-table-fixed">
            <view class="zb-table-tbody">
              <view  :class="['item-tr',highlight&&isHighlight(item,index)?'current-row':'',
											item.isCurCompare?'current-compare-row':'', !columns[0].type? 'fst-col-no-val': 'fst-col-exist-val']"
                     @click="rowClick(item,index)"
										 @longpress="(e)=>trLongPress(e, item)"
										 @touchstart="trTouchStart"
                     v-for="(item,index) in transData" :key="item.key" 
										 :hover-class="hover?'hover-class': ''">
								<table-hover-button :isShowHoverBtn="item.isShowHoverBtn" :offsetLeft="offsetLeft" 
									:triggerUpdate="triggerUpdate" :trInfo="item" @delete="deleteTr"></table-hover-button>
                <view
                    :style="[{
                      left:`${ite.left}px`,
                      width:`${ite.width?ite.width:'100'}px`,
...
export default {
  ...
  mounted(){
		...
		query.select('.item-tr').boundingClientRect(data => {
